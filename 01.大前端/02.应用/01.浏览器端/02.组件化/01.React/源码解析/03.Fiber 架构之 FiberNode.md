...
上面罗列了 FiberNode 的一些属性，现在具体看看 fiber.tag。fiber.tag 表示了 FiberNode 的类别，React 对每一类别的 FiberNode 会采取不同的处理方式，例如类组件和函数组件在使用和 React 源码内部都有着很大的不同。Fiber 节点都有哪些 tag 呢？详见以下代码：
