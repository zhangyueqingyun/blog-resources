...
为了重复利用 Fiber 节点，React 创建了两颗 Fiber 树（current 树和 workInProgress 树）交替工作。这些之后的章节将会详细介绍，今天我们的主要内容为 Fiber 树的节点：FiberNode。

## 三、Fiber 树的节点

### 3.1 FiberNode

FiberNode 是 Fiber 节点的类实现，下面介绍一下 FiberNode 的属性：
...
4. stateNode：children 对应的 ReactElement ；其他状态信息。
