...
在 create Fiber 之上还有一些创建特定类别的工厂函数，例如 createFiberFromElement (创建元素类对应的 Fiber)，createHostRootFiber(创建 HostRoot)，createFiberFromProfiler(创建 Profiler 对应的 Fiber)，每一个工厂函数会设置该类别 Fiber 所对应的各种属性,例如 elementType 等。本章我们将举一个例子来讲解。

#### 4.1 创建 ReactElement 对应的 Fiber

createFiberFromElement 是创建 ReactElement 所对应的 Fiber 的工厂函数。它调用了另外一个负责根据 type 和 props 来创建 fiber 的函数。
...
createFiberFromTypeAndProps 会根据不同的 type 来为不同的 ReactElement 创建 fiber ，源码解析如下：
