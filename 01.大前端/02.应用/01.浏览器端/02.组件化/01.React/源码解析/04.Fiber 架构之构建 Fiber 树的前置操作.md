...
ReactDOMRoot.render 会渲染我们定义好的元素树，在渲染的过程中，react 会初始化或更新元素树的内部表示 - Fiber 树。
...
ReactDOMRoot.render 调用 updateContainer 函数完成对该容器节点的更新。

当未初始化过 Fiber 树时, updateContainer 会进行 fiber 树的创建工作；当更新事件发生时， updateContainer 会进行 fiber 树的更新操作。

下面我们将详细看一下 Fiber 树的初始化。

## 二、Fiber 树的初始化


#### 2.1 创建 fiber 树前的准备工作

创建 fiber 树之前，react 做了比较多的准备工作，其中涉及了 updateContainer、scheduleUpdateOnFiber、ensureRootIsScheduled、performConcurrentWorkOnRoot、renderRootConcurrent、prepareFreshStack、createWorkInProgress 这些函数的调用。

其中 updateContainer、scheduleUpdateOnFiber、ensureRootIsScheduled、performConcurrentWorkOnRoot、renderRootConcurrent 处理了更新的优先级、lane、context等数据。prepareFreshStack、createWorkInProgress 创建了每一个任务执行的最小数据单元：workInProgress。
